...
const CategoryNavbar = () => {
  ...
  const areFiltersActive = useMemo(() => {
    return !!(categoryParam || dealsParam || searchParam || minPriceParam || maxPriceParam);
  }, [categoryParam, dealsParam, searchParam, minPriceParam, maxPriceParam]);
          ...
          {loading ? (
            ...
            mainCategories.map(category => (
              <Link
                ...
                className="group flex-shrink-0 flex flex-col items-center space-y-1 text-center transition-transform duration-200 ease-in-out hover:-translate-y-1"
              >
                <div
                  className="w-16 h-16 rounded-full overflow-hidden border-2 border-gray-200 group-hover:border-primary group-hover:shadow-lg transition-all duration-300"
                >
                  <img
